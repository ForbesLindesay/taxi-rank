...
process.on('message', message => {
  Promise.resolve(null).then(() => handleMessage(message)).then(result => {
    process.send({status: 0, value: result});
  }, err => {
    if (err.webdriverError) {
      process.send(err.webdriverError);
    } else {
      process.send({status: 13, value: {message: err.stack}});
    }
  });
});
